...
/** DELETE /[id]/apply  {state} => {message: state} */

router.delete("/:id/retract", authRequired, async function(req, res, next) {
  try {
    const state = req.body.state || "applied";
    await Job.retract(req.params.id, req.username, state);
    return res.json({ message: state });
  }
