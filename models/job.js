...
class Job {
  ...
  /** Retract job application: update db, returns undefined. */

  static async retract(job_id, username) {
    const result = await db.query(
        `SELECT job_id
          FROM applications 
    ...
    if (result.rows.length === 0) {
      let notFound = new Error(`No application record found for '${id}, ${username}`);
      notFound.status = 404;
      throw notFound;
    }
